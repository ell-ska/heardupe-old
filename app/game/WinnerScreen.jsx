...
const WinnerScreen = ({ outcome, seconds, image, title, name, release }) => {
    ...
    const message = outcome === 'won' ? `You got the song from ${seconds} seconds` : "You can't know all of them..."

    return (
        <div className="game-over">
            <h3>{exclamation}</h3>
            <span>{message}</span>
            <Image src={image.url} width={image.width} height={image.height} alt={`image of ${name}`}></Image>
