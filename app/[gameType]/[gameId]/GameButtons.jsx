...
const SkipButton = ({ stage, handleNextStage, levelEnded, gameEnded, currentSongSpotifyLink }) => {
    ...
    return levelEnded && !gameEnded ? <ListenOnSpotify /> : gameEnded ? <Home /> : <Skip />
}

const NextLevelButton = ({ handleNextLevel, level, levelEnded, gameEnded, handleFinalScore, resetGame }) => {
    ...
    const NextSong = () => {
        return <button className="button button--outline" onClick={handleNextLevel}>{!levelEnded ? 'Reveal answer' : 'Next song'}</button>
    }
    ...
    return gameEnded ? <PlayAgain /> : level === 9 && levelEnded ? <FinalScore /> : <NextSong />
}
